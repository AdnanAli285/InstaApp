...
protocol UserDataSource {
    func getUsers(by index: Int) async -> [User]
}

struct UserLocalDataSource: UserDataSource {
    func getUsers(by index: Int) async -> [User] {
        let json: String
        ...
        default:
            ...
            return (1...10).compactMap { number in
                let id = start + number
                let image = "https://i.pravatar.cc/300?u=\(id)"
                return User(id: id, name: "User \(id)", imageUrl: image)
            }
        }
        
        // Runs only when for defined cases, will not run for the randomly generated Users
